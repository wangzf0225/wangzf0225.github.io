...
{% end hightlight %}
在registerExtenderCallbacks方法的定义中将这个插件注册成为一个MessageEditorTabFactory，即添加一行：callbacks.registerMessageEditorTabFactory(self)。然后定义createNewInstance>方法，根据开发文档的定义，这个方法需要返回一个实例化后的IMessageEditorTab对象。于是我们在这里初始化了一个叫MakeTabs的类，后面我们只需要按照接口规范重新定义这个类即可。
